<script>
	...
	$: slotsFilled =
		$craftingState.fusionCrafting.slot1 &&
		$craftingState.fusionCrafting.slot2 &&
		$craftingState.fusionCrafting.slot3 &&
		$craftingState.fusionCrafting.slot4 &&
		$craftingState.fusionCrafting.slot5 &&
		$craftingState.fusionCrafting.slot6 &&
		$craftingState.fusionCrafting.slot7 &&
		$craftingState.fusionCrafting.slot8 &&
		$craftingState.fusionCrafting.slot9 &&
		$craftingState.fusionCrafting.slot10;
	$: {
		if (
			!(
				$craftingState.fusionCrafting.slot1 ||
	...
	async function removeAll() {
		selectedRarity = null;
		$craftingState.fusionCrafting = JSON.parse(JSON.stringify(CRAFTING_STATE_INIT.fusionCrafting));
	...
	async function addGp(assets) {
		for (let index = 0; index < assets.length; index++) {
			for (let index2 = 0; index2 < 10; index2++) {
				if (!$craftingState.fusionCrafting[`slot${index2 + 1}`]) {
					$craftingState.fusionCrafting[`slot${index2 + 1}`] = assets[index];
					break;
				}
			}
		}
	...
	async function waitForAttemptResult() {
		...
		let craft;
		while (!data) {
			const craftLog = await greenRabbitApi.fetchFusionOutcome($activeUser.accountName);
	...
	const MAX_DAILY_GP_FUSIONS = 10; // Temporary
	$: {
		if (limits) {
			if (limits.limit_start + 86400 > $now)
				remainingAttempts = TESTNET
					? 1000 - limits.gp_fusions
					: MAX_DAILY_GP_FUSIONS - limits.gp_fusions;
		...
		if (browser) {
			if (!craftingConfig) {
				...
				costs = {
					none: 0,
					Common: Number(
						craftingConfig.gp_fusion_fees.filter((e) => e.rarity == 'Common')[0].cost.split(' ')[0]
					),
					Uncommon: Number(
						craftingConfig.gp_fusion_fees
							.filter((e) => e.rarity == 'Uncommon')[0]
							.cost.split(' ')[0]
					),
					Epic: Number(
						craftingConfig.gp_fusion_fees.filter((e) => e.rarity == 'Epic')[0].cost.split(' ')[0]
					),
					Legendary: Number(
						craftingConfig.gp_fusion_fees
							.filter((e) => e.rarity == 'Legendary')[0]
							.cost.split(' ')[0]
					),
					Mythic: Number(
						craftingConfig.gp_fusion_fees.filter((e) => e.rarity == 'Mythic')[0].cost.split(' ')[0]
					)
				};
			}
		}
		...
		<div class="grid">
			{#each Array(10) as _, i}
				<div in:fade={{ delay: animDelayBase * (i + 1) }}>
					<FusionCard
						slot={`slot${i + 1}`}
						nftData={$craftingState.fusionCrafting[`slot${i + 1}`]}
						type="fusionCrafting"
						cardType="GREENPRINT"
						...
						maxSelectCount={10 - numSlotsFilled}
					/>
				</div>
			{/each}
